...
import { getCart } from '@/apis/purchase.apis'
import { genNameId } from '@/utils/utils'

export const Header = () => {
  const [qttProductsInCart, setQttProductsInCart] = useState(0)
  ...
  const productsInCart = cartData?.data.data

  useEffect(() => {
    if (isSuccess) {
      if (productsInCart && productsInCart.length > 0) {
        setQttProductsInCart(productsInCart.length)
      }
    }
  }, [isSuccess, productsInCart])

  const handleClickProductInCart = (productId: string, name: string) => () => {
    navigate(`${path.home}${genNameId({ name: name, id: productId })}`)
  }
  ...
  return (
    <div className='relative z-[1] bg-[#f53d2d] bg-[linear-gradient(-180deg,#f53d2d,#f63)]'>
      <div className='container'>
        ...
        <div className='flex justify-between h-[5.3125rem] pt-4 pb-[0.625rem]'>
          ...
          <Popover
            className='flex-1 flex justify-center items-center mx-[0.625rem]'
              ...
              <div className='bg-white z-1 overflow-hidden shadow-[0_1px_3.125rem_0_rgba(0,0,0,.2)] border border-gray-300 w-[25rem] rounded-sm flex flex-col text-sm'>
                {!isAuthenticated && (
                  <div className='py-[3.75rem] text-center'>
                    <div className='bg-[url("/src/assets/images/no-product.png")] bg-center bg-no-repeat bg-cover inline-block size-[100px]'></div>
                    <div className='capitalize font-normal leading-tight'>Chưa có sản phẩm</div>
                  </div>
                )}
                {isAuthenticated && productsInCart && (
                  <>
                    {productsInCart.length > 0 && (
                      <>
                        ...
                        {productsInCart.slice(0, 5).map((product) => (
                          <div
                            onClick={handleClickProductInCart(product.product._id, product.product.name)}
                            className='overflow-hidden p-[0.625rem] flex hover:bg-[#f8f8f8]'
                            key={product._id}
                          >
                            <img
                            ...
                            />
                            <div className='truncate flex-grow mr-10 text-[#000000cc]'>{product.product.name}</div>
                            <div className='text-orange'>
                              <div className='flex items-baseline flex-shrink truncate'>
                                <span className='mr-px text-xs'>₫</span>
                                <span className='text-sm truncate'>{product.price.toLocaleString('vi-VN')}</span>
                              </div>
                            </div>
                          </div>
                        ))}
                        <div className='h-[3.75rem] p-[0.625rem] flex justify-between items-center capitalize'>
                          {productsInCart.length - 5 > 0 && (
                            <div className='flex items-center gap-1 text-xs text-[#000000cc]'>
                              <span>{productsInCart.length - 5}</span>
          ...
          >
            <div className='py-[0.625rem]'>
              <Link to={path.cart} className='flex justify-center items-center relative'>
                <svg
                  viewBox='0 0 26.6 25.6'
                  className='fill-white size-[1.625rem] stroke-white text-white mr-[0.625rem]'
                  stroke='1.5'
                >
                ...
                {qttProductsInCart > 0 && (
                  <div className='bg-white -top-[0.425rem] -right-[0.6rem] text-sm leading-tight text-orange px-[0.3125rem] min-w-[1.4875rem] absolute flex items-center justify-center rounded-full border-2 border-orange'>
                    {qttProductsInCart}
                  </div>
                )}
              </Link>
