  ...
  return (
    <div className='bg-[#f5f5f5] py-5'>
      <div className='container'>
        ...
        {isSuccess && (
          <>
            <BreadCrumb productName={productDetailData.data.data.name} />
            <section className='flex mt-5 bg-white rounded-md shadow-sm'>
              <figure className='flex-shrink-0 w-[480px] p-[0.9375rem] flex-col'>
                <div className='flex flex-col'>
                  <div className='relative w-full pt-[100%]'>
                  ...
                  <div className='relative my-[0.3125rem] grid grid-cols-5 gap-[0.625rem]'>
                    <button className='absolute z-10 cursor-pointer outline-none tracking-normal font-light flex items-center justify-center w-5 h-10 bg-[#0003] text-white left-0 top-1/2 -translate-y-1/2'>
                      <svg
                        xmlns='http://www.w3.org/2000/svg'
                        fill='none'
                        viewBox='0 0 24 24'
                    ...
                    <button className='absolute z-10 cursor-pointer outline-none tracking-normal font-light flex items-center justify-center w-5 h-10 bg-[#0003] text-white right-0 top-1/2 -translate-y-1/2'>
                      <svg
                        xmlns='http://www.w3.org/2000/svg'
                        fill='none'
                        viewBox='0 0 24 24'
                    ...
                    {productDetailData.data.data.images.slice(0, 5).map((image, index) => {
                      const isActive = index === 0
                      return (
                        <div className='relative w-full pt-[100%] cursor-pointer' key={image}>
                          <img
                            className='absolute top-0 left-0 w-full h-full object-contain object-center bg-white align-middle'
                            src={image}
                            alt={productDetailData.data.data.name}
              ...
              <section className='flex flex-1 flex-col pt-5 pl-5 pr-[1.5625rem] text-[#000000cc]'>
                <h3 className='text-xl font-medium m-0 leading-6 break-words line-clamp-2'>
                  {productDetailData.data.data.name}
                </h3>
                <div className='flex items-center min-h-[1.55rem] mt-[0.625rem]'>
                  <button className='bg-transparent outline-none border-none text-orange cursor-pointer flex items-baseline gap-[0.3125rem] pr-[0.9375rem]'>
                    <div className='border-b border-current'>{productDetailData.data.data.rating.toFixed(1)}</div>
                    <ProductRating />
                  </button>
                  ...
                  <button className='bg-transparent border-l border-[#00000024] outline-none text-orange cursor-pointer flex items-baseline px-[0.9375rem] gap-[0.3125rem]'>
                    <div className='text-[#222] text-base leading-[1.2rem]'>
                      {formatNumberToSocialStyle(productDetailData.data.data.sold)}
                    </div>
                ...
                <div className='flex bg-[#fafafa] px-5 py-[0.9375rem] items-center flex-wrap min-h-[2.25rem] mt-[0.625rem]'>
                  {productDetailData.data.data.price_before_discount - productDetailData.data.data.price > 0 && (
                    <div className='text-base leading-[1.2rem] text-[#929292] line-through mr-[0.625rem]'>{`₫${productDetailData.data.data.price_before_discount.toLocaleString('vi-VN')}`}</div>
                  )}
                  <div className='flex items-center'>
                    <div className='text-orange font-medium text-3xl'>{`₫${productDetailData.data.data.price.toLocaleString('vi-VN')}`}</div>
                    {productDetailData.data.data.price_before_discount - productDetailData.data.data.price > 0 && (
                      <div className='bg-orange text-white px-1 py-0.5 rounded-sm uppercase font-semibold text-xs leading-none ml-[0.9375rem]'>
                        {`${Math.round(
                          ((productDetailData.data.data.price_before_discount - productDetailData.data.data.price) /
                            productDetailData.data.data.price_before_discount) *
                            100
                        )}% giảm`}
                      </div>
                    )}
                    ...
                    <Popover
                      ...
                      renderPopover={
                        <div className='relative text-sm leading-[1.375rem] w-[29.6875rem] max-h-[12.5rem] overflow-y-auto break-words text-left text-[#000c] p-5 bg-white border border-[#00000017] rounded-sm shadow-sm box-content'>
                          Miễn phí Trả hàng trong 15 ngày nếu Đổi ý (hàng trả phải còn nguyên seal, tem, hộp sản phẩm),
                          áp dụng cho một số sản phẩm nhất định. Ngoài ra, tại thời điểm nhận hàng, bạn có thể đồng kiểm
                          và được trả hàng miễn phí.
                ...
                <div className='flex items-center text-[#222] p-1 mb-[1.5625rem] text-sm leading-[1.05rem]'>
                  ...
                  <div className='flex items-center text-[#555]'>
                    ...
                    <div>{`${productDetailData.data.data.quantity} sản phẩm có sẵn`}</div>
                  </div>
                </div>
                <div className='flex'>
                ...
                <section className='flex-1 p-[1.5625rem] pb-[0.625rem]'>
                  ...
                  <div className='m-[0.9375rem] mt-[1.875rem]'>
                    <div
                      ...
                      dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(productDetailData.data.data.description) }}
                    />
                  </div>
                </section>
              </div>
